...
echo "usage: ./tpganalysis_jenkins_2025.sh $sqlite1 $sqlite2 $week $year $(getconf _NPROCESSORS_ONLN)"
...
#./runTPGbatchLC_jenkins_2025.sh jenkins $reference $dataset $GT $nevents $sqlite1 $(getconf _NPROCESSORS_ONLN) &
#wait
./runTPGbatchLC_jenkins_2025.sh jenkins $reference $dataset $GT $nevents $sqlite2 $(getconf _NPROCESSORS_ONLN) &
wait
fi
#cp addhist_jenkins_2025.sh log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite1}_batch/.
#pushd log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite1}_batch/
#./addhist_jenkins_2025.sh ${sqlite1} &
#popd
cp addhist_jenkins_2025.sh log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite2}_batch/.
pushd log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite2}_batch/
./addhist_jenkins_2025.sh ${sqlite2} &
popd
wait
...
./validationplots_jenkins_2025.sh $sqlite1 $sqlite2 $reference $week $year
