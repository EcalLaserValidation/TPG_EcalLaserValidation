...
#./runTPGbatchLC_jenkins_2023.sh jenkins $reference $dataset $GT $nevents $sqlite1 $(getconf _NPROCESSORS_ONLN) &
#wait
...
#popd
cp addhist_jenkins_2023.sh log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite2}_batch/.
pushd log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite2}_batch/
./addhist_jenkins_2023.sh ${sqlite2} &
popd
wait
