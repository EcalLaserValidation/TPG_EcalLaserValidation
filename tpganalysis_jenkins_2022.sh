...
#./runTPGbatchLC_jenkins_2022.sh jenkins $reference $dataset $GT $nevents $sqlite1 $(getconf _NPROCESSORS_ONLN) &
#wait
./runTPGbatchLC_jenkins_2022.sh jenkins $reference $dataset $GT $nevents $sqlite2 $(getconf _NPROCESSORS_ONLN) &
wait
fi
#cp addhist_jenkins_2022.sh log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite1}_batch/.
#pushd log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite1}_batch/
#./addhist_jenkins_2022.sh ${sqlite1} &
#popd
cp addhist_jenkins_2022.sh log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite2}_batch/.
pushd log_and_results/${reference}_${datasetpath}_LC_IOV_${sqlite2}_batch/
./addhist_jenkins_2022.sh ${sqlite2} &
popd
wait
...
if [[ `wget -S --spider https://cmssdt.cern.ch/SDT/public/EcalLaserValidation/TPG_EcalLaserValidation/TPGAnalysis_${year}_${week}_${sqlite1}_${sqlite2}/TPGAnalysis/${year}/tpg_validationplots/week_${week}/run_${sqlite1}_${sqlite2}_ref_${reference}/newhistoTPG_${sqlite1}.root 2>&1 | grep 'HTTP/1.1 200 OK'` ]]
	then 
		...
		if [[ `wget -S --spider https://ecaltrg.web.cern.ch/ecaltrg/TPG/ReferenceNTuples/newhistoTPG_${sqlite1}.root  2>&1 | grep 'HTTP/1.1 200 OK'` ]]
		then
...
./validationplots_jenkins_2022.sh $sqlite1 $sqlite2 $reference $week $year
